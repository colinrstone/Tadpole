...
namespace QuickSDL {
	...
	Graphics::Graphics() {
		...
		// Get the player's screen resolution
		SDL_DisplayMode currentDisplayMode;
		if (SDL_GetCurrentDisplayMode(0, &currentDisplayMode) != 0) {
			// Handle error if SDL_GetCurrentDisplayMode fails
			printf("Error getting display mode: %s\n", SDL_GetError());
			return;
		}

		mLogicalWidth = 1920; // Set your preferred logical width
		mLogicalHeight = 1080; // Set your preferred logical height

		// Calculate scaling factors based on the player's screen resolution
		float scaleX = static_cast<float>(SCREEN_WIDTH) / mLogicalWidth;
		float scaleY = static_cast<float>(SCREEN_HEIGHT) / mLogicalHeight;

		// Set the logical size and scaling
